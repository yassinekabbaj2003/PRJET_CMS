...
<a href="<?php echo esc_url($post->link); ?>" class="vc_read_more"
   title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to %s', 'xstore' ), $post->title_attribute ) ); ?>"<?php echo wp_specialchars_decode($this->link_target); ?>><?php esc_html_e( 'Read more', 'xstore' ) ?></a>
<?php endif; ?>