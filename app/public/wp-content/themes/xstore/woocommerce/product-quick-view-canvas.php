<?php
/**
 * The Template for displaying product quick view content (off-canvas type).
 *
 * Override this template by copying it to yourtheme/woocommerce/product-quick-view-canvas.php
 *
 * @author 	   Stas
				...
				switch ( $item ) {
					...
					case 'quick_categories':
						...
						if ( $estimated_delivery_exists ) {
                            ...
                            $estimated_delivery->args['in_quick_view'] = false;
                        }
						break;
					case 'quick_share':
						?>
					...
					case 'quick_short_descr':
						...
						break;
					case 'product_link':
						if ( ! etheme_get_option( 'quick_descr', 1 ) ) : ?>
					...
					case 'quick_add_to_cart':
                        ...
                        if ( $element_options['built_in_waitlist'] && class_exists('XStoreCore\Modules\WooCommerce\XStore_Waitlist')) {
                            $element_options['built_in_waitlist_instance'] = XStoreCore\Modules\WooCommerce\XStore_Waitlist::get_instance();
                            add_filter('woocommerce_get_stock_html', array($element_options['built_in_waitlist_instance'], 'print_button_single_stock_replace'), 100, 2);
//                            add_action('woocommerce_after_single_product', array($element_options['built_in_waitlist_instance'], 'output_form_script'));
                        }
