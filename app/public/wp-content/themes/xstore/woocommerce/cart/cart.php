...
$elementor_cart_builder = apply_filters('etheme_elementor_cart_page', false);
...
$product_quantity_size = apply_filters('etheme_cart_table_product_quantity_size', 'size-sm');
$product_quantity_style = apply_filters('etheme_cart_table_product_quantity_style', 'square');
if ( $product_quantity_size )
    $product_quantity_style .= ' ' . $product_quantity_size;
...
<?php endif; ?>
    ...
    <form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
        ...
        <div class="table-responsive">
            <table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
                ...
                <tr>
                    ...
                    <th class="product-price mob-hide"><?php esc_html_e( 'Price', 'xstore' ); ?></th>
                    <?php if ( $show_sku_field ) : ?>
                        <th class="product-sku mob-hide"><?php esc_html_e( 'SKU', 'xstore' ); ?></th>
                    <?php endif; ?>
                    <th class="product-quantity mob-hide"><?php esc_html_e( 'Quantity', 'xstore' ); ?></th>
                    <th class="product-subtotal mob-hide" colspan="2"><?php esc_html_e( 'Subtotal', 'xstore' ); ?></th>
                </tr>
                </thead>
                <tbody>
                        ...
                        <tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
                            ...
                            <td class="product-details">
                                <div class="cart-item-details">
                                    ...
                                    ?>
                                    <div class="product-price dt-hide">
                                        <?php
                                        echo '<div class="product-price-quantity">';
                                            if ( $_product->is_sold_individually() ) {
                                                ...
                                                $product_quantity = woocommerce_quantity_input( array(
                                                    'input_id'     => uniqid( 'quantity_duplicated_' ),
                                                    'input_name'  => "cart[{$cart_item_key}][qty_duplicated]",
                                            ...
                                            echo apply_filters( 'woocommerce_cart_item_quantity', str_replace('{{quantity_type}}', $product_quantity_style, $product_quantity), $cart_item_key, $cart_item ); // PHPCS: XSS ok.
                                    ...
                                    <?php if ( $show_sku_field ) : ?>
                                        <div class="product-sku dt-hide">
                                            <?php
                                            if ( $_product->get_sku() ) {
                                                echo sprintf(esc_html__('SKU: %s', 'xstore'), '<span>'.esc_html( ( $sku = $_product->get_sku() ) ? $sku : esc_html__( 'N/A', 'xstore' ) ).'</span>');
                                            }
                                            ?>
                                        </div>
                                    <?php endif; ?>

                                    <div class="product-subtotal dt-hide">
                                        <?php
                                            echo sprintf(__('Subtotal: %s', 'xstore'), '<span>'.apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ), $cart_item, $cart_item_key ).'</span>');
                                        ?>
                                    </div>
                            ...
                            <td class="product-price mob-hide" data-title="<?php esc_attr_e( 'Price', 'xstore' ); ?>">
                                <?php
                            ...
                            <?php if ( $show_sku_field ) : ?>
                                <td class="product-sku mob-hide" data-title="<?php esc_attr_e( 'SKU', 'xstore' ); ?>">
		                            <?php
                                        if ( $_product->get_sku() ) {
                            ...
                            <td class="product-quantity mob-hide" data-title="<?php esc_attr_e( 'Quantity', 'xstore' ); ?>">
                                <?php
                                if ( $_product->is_sold_individually() ) {
                                    $product_quantity = sprintf( '1 <input type="hidden" name="cart[%s][qty]" value="1" />', $cart_item_key );
                                } else {
                            ...
                            <td class="product-subtotal mob-hide" data-title="<?php esc_attr_e( 'Subtotal', 'xstore' ); ?>">
                                <?php
        ...
        <div class="actions clearfix">
            ...
            <?php if ( wc_coupons_enabled() && !apply_filters('etheme_elementor_cart_page', false) ) : $first_col = 6; ?>
    ...
    <?php if ( !$elementor_cart_builder && get_query_var('et_is-cart-checkout-advanced', false ) && get_query_var('et_cart-checkout-layout', 'default') == 'separated') {
...
<?php do_action( 'woocommerce_before_cart_collaterals' ); ?>
    ...
    <div class="<?php if ( !$elementor_cart_builder ) : ?>col-md-5 <?php endif; ?>">
        <div class="cart-order-details">
                <?php do_action('etheme_woocommerce_cart_before_collaterals'); ?>
...
<?php if ( $cross_sells_after_content && !$elementor_cart_builder ) woocommerce_cross_sell_display(); ?>
