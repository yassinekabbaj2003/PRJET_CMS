...
$heading = apply_filters( 'woocommerce_product_cross_sells_products_heading', __( 'You may be interested in&hellip;', 'xstore' ) );

if ( $cross_sell_type != 'widget' ) {
	echo '<h2 class="products-title cross-sell-products-title"><span>' . $heading . '</span></h2>';
}
...
if ( $cross_sell_type == 'widget' ) {
	...
	echo '<div class="sidebar-slider">' . etheme_slider( $args, 'product', apply_filters('cross_sell_slides_slider_args', $slider_args) ) . '</div>';
	...
	etheme_slider( $args, 'product', apply_filters('cross_sell_slides_slider_args', $slider_args) );
} else {
