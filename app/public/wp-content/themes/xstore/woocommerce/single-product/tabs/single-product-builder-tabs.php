...
$element_options['product_tabs_type'] = apply_filters('etheme_single_product_tabs_type', etheme_get_option( 'product_tabs_type_et-desktop', 'underline' ));
$element_options['product_tabs_style'] = apply_filters('etheme_single_product_tabs_style', etheme_get_option( 'product_tabs_style_et-desktop', 'horizontal' ));
...
$element_options['product_tabs_closed'] = $element_options['product_tabs_type'] == 'accordion' &&
    ((!apply_filters('etheme_elementor_theme_builder', false) && !etheme_get_option( 'product_first_tab_opened_et-desktop', 0 )) ||
        (apply_filters('etheme_elementor_theme_builder', false) && !apply_filters('etheme_single_product_tabs_first_tab_opened', false )));
$element_options['product_tabs_opened'] = $element_options['product_tabs_type'] == 'accordion' &&
    ((!apply_filters('etheme_elementor_theme_builder', false) && get_theme_mod('product_tabs_opened_et-desktop', 0)) || (apply_filters('etheme_elementor_theme_builder', false) && apply_filters('etheme_single_product_tabs_opened', false )));
$element_options['product_tabs_scroll'] = apply_filters('etheme_single_product_tabs_scroll', etheme_get_option( 'product_tabs_scroll_et-desktop', 0 ));
...
if ( $element_options['product_tabs_type'] == 'accordion' && apply_filters('etheme_single_product_tabs_force_load_assets', true) ) {
	etheme_enqueue_style('toggles-by-arrow');
}

$element_options['classes'] = apply_filters('etheme_single_product_tabs_wrapper_classes', $element_options['classes']);

$element_options['ul_classes'] = '';
...
if ( ! empty( $product_tabs ) ) : ?>
	...
	<div class="woocommerce-tabs et-clearfix et_element wc-tabs-wrapper <?php echo esc_attr( $element_options['classes'] ); ?>" data-title="<?php esc_attr_e('Tabs', 'xstore'); ?>">
		<div class="tabs wc-tabs<?php echo esc_attr( $element_options['ul_classes'] ); ?>" role="tablist">
			<?php foreach ( $product_tabs as $key => $product_tab ) : if ( !isset($product_tab['title'])) continue; ?>
		...
		foreach ( $product_tabs as $key => $product_tab ) : if ( !isset($product_tab['title'])) continue; ?>
...
if ( apply_filters('etheme_should_reinit_swiper_script', false) ) : ?>
    <script>jQuery(document).ready(function(){
            ...
            if ( etTheme.singleProductBuilder !== undefined )
                etTheme.singleProductBuilder();
        });</script>
<?php endif;
