...
add_action('init', function () {
	...
	switch ($cropping) {
        case 'uncropped':
            break;
        case 'custom':
            $wc_mini_sizes['width']          = max( 1, get_option( 'woocommerce_thumbnail_cropping_custom_width', '4' ) );
            $wc_mini_sizes['height']         = max( 1, get_option( 'woocommerce_thumbnail_cropping_custom_height', '3' ) );
            break;
        default:
            $cropping_split = explode( ':', $cropping );
            $wc_mini_sizes['width']          = max( 1, current( $cropping_split ) );
            $wc_mini_sizes['height']         = max( 1, end( $cropping_split ) );
            break;
    }
