			...
			class Kirki_Installer_Section extends WP_Customize_Section {
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php echo sprintf(esc_html__( 'To fully utilize the customization options available with the XStore theme, please install the %s Core plugin. Thank you!', 'xstore' ), apply_filters('etheme_theme_label', 'XStore')); ?>
					</p>
                    <?php // esc_url_raw( $this->get_plugin_install_url() ) ?>
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( admin_url('admin.php?page=et-panel-plugins&plugin=et-core-plugin') ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'xstore' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Install Now', 'xstore' ); ?>
					</a>
					<?php
				}
				/**
				...
				protected function activate_button() {
					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'xstore' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'xstore' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Activate Now', 'xstore' ); ?>
					</a>
					<?php
				}
				/**
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="kirki" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'xstore' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'xstore' ); ?>">
						<?php esc_html_e( 'Don\'t show this again', 'xstore' ); ?>
					</a>
					...
					jQuery( document ).ready( function() {
						jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
							...
							<?php if ( $show_confirm ) : ?>
								if ( ! confirm( '<?php esc_html_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'xstore' ); ?>' ) ) {
									return;
								}
							<?php endif; ?>
							jQuery.post( ajaxurl, {
