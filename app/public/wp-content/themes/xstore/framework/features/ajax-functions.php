...
//        }
		...
		if ( isset( $_POST[ 'content' ] ) && ! empty( $_POST[ 'content' ] ) ) {
			...
			$content = do_shortcode( $content ) . '[/' . esc_attr($_POST[ 'element' ]) . ']';
		} else {
			$content = '';
		}
		echo do_shortcode( '[' . esc_attr($_POST[ 'element' ]) . ' ' . $atts . ' ]' . $content );
		die();
	}
}
...
if ( ! function_exists( 'et_ajax_blog_element' ) ) {
	function et_ajax_blog_element(){
		...
		echo do_shortcode( '[' . esc_attr($_POST[ 'element' ]) . ' ' . $atts . ' paged="' . esc_attr($_POST[ 'paged' ]) . '" html_type="true" ]' );
		die();
	}
}