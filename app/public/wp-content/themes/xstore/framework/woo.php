...
if ( ! function_exists( 'etheme_template_hooks' ) ) {
	function etheme_template_hooks() {
	    ...
	    $is_spb = get_query_var( 'etheme_single_product_builder', false );
		if ( $is_spb ) {
            ...
            if ( get_theme_mod('attributes_after_description', false) ) {
                add_action( 'woocommerce_single_product_summary', 'etheme_display_product_attributes', 20 );
		...
		if ( ! $is_spb ) {
			...
			if ( etheme_get_option( 'reviews_position', 'tabs' ) == 'outside' ) {
				...
				if ( ( get_theme_mod('product_reviews_collapsed_et-mobile', false) && get_query_var('is_mobile', false) ) ||
				    ( get_theme_mod('product_reviews_collapsed_et-desktop', false) && !get_query_var('is_mobile', false) ))
		...
		if ( get_query_var('et_brands', true) && etheme_get_option( 'show_brand', 1 ) ) {
		    $brands_location = etheme_get_option( 'brands_location', 'sidebar' );
		    if ($brands_location == 'content' ) {
                if ( ! $is_spb ) {
		    ...
		    elseif ( $brands_location == 'under_content' ) {
...
//        }
		...
		// plugins compatibles
//        add_filter('mvx_load_default_vendor_list', '__return_true');
//		add_filter('mvx_load_default_vendor_store', '__return_true');
        
        add_action('mvx_before_main_content', 'woocommerce_breadcrumb');
        add_action('mvx_before_main_content', function(){
            echo '<div class="container content-page products-hover-only-icons">';
        });
		
		add_action( 'mvx_after_main_content', function (){
		    echo '</div>';
        } );
		
		add_filter('mvx_show_page_title', '__return_false');
...
//            $cart_checkout_class = Etheme_WooCommerce_Cart_Checkout::get_instance();
        ...
        if ( get_theme_mod('product_empty_rating', false) ) {
            add_filter('woocommerce_product_get_rating_html', 'etheme_product_empty_rating', 10, 3);
        }

        // hide it in code instead of CSS
        add_filter('etheme_checkout_form_shipping_title', '__return_false');
	}
}

// outside of 'init' action because these hooks don't work from there
...
if ( !function_exists('etheme_sales_booster_cart_checkout_progress_bar') ) {
    function etheme_sales_booster_cart_checkout_progress_bar($force_load_assets = true) {
        if ( false !== $force_load_assets ) // empty condition for shortcode
...
add_action( 'wp_ajax_etheme_format_price', 'etheme_format_price');
add_action( 'wp_ajax_nopriv_etheme_format_price', 'etheme_format_price');

if ( !function_exists('etheme_format_price') ) {
    function etheme_format_price() {
...
if ( !function_exists('etheme_bought_together') ) {
    function etheme_bought_together($args = array()) {
        ...
        if ( $is_spb ) {
            $default_args['title'] = get_theme_mod('single_product_bought_together_products_title', $default_args['title']);
        }
...
if ( !function_exists('etheme_display_product_attributes') ) {
    function etheme_display_product_attributes() {
        global $product;
...
if(!function_exists('etheme_ajax_products')) {
	function etheme_ajax_products( $args = array() ){
		...
		$args = Array(
			...
			'paged' => $attr['paged'],
			'orderby' => $attr['orderby']?$attr['orderby'] : '',
...
//	    }
        ...
        if(isset($attr['brands']) && !empty($attr['brands'])){
            $args['tax_query'][] = array(
                'taxonomy' => 'brand',
                'field' => 'term_id',
                'operator' => 'IN'
        ...
        if ( isset($attr['show_excerpt']) )
			$woocommerce_loop['show_excerpt'] = $attr['show_excerpt'];
		...
		if ( $products->have_posts() ) :
			if ( wc_get_loop_prop( 'total' ) ) {
				...
				<?php while ( $products->have_posts() ) : $products->the_post(); ?>
					...
					<?php
					if ( isset( $attr['limit'] ) && isset($attr['loaded']) ) {
						if (
                            $_i >= $attr['limit']
//                            || (strval($attr['paged']) * strval($attr["per-page"])) - $_i == strval($attr['limit'])

                            || strval($attr['loaded'] + $_i) >= ( strval($attr['limit']) )
						) {
							break;
						}
						$_i++;
...
if ( !function_exists('et_cart_checkout_breadcrumbs') ) {
	function et_cart_checkout_breadcrumbs() {
        ...
        // removed in v9.2.6 because cannot disable countdown when Elementor Cart/Checkout builder and mostly
        // customers use special cart checkout breadcrumbs in common with countdown
//        else {
//            if (count($countdown)) :
//                echo '<div ' . $countdown['attributes'] . '>' . $countdown['content'] . '</div>';
//            endif;
...
add_action('wp_loaded', function() {
    ...
    add_filter('woocommerce_add_to_cart_redirect', function($url) {
		if (isset($_REQUEST['et_buy_now']) && (isset($_REQUEST['et_buy_now_builder_element']) || etheme_get_option('buy_now_btn',0))) {
...
if ( ! function_exists( 'etheme_shop_filters_sidebar' ) ) {
	function etheme_shop_filters_sidebar() {
        if ( !apply_filters('etheme_shop_filters_enabled', true) ) return;
		if ( is_active_sidebar( 'shop-filters-sidebar' ) ):
		    if ( apply_filters('etheme_filter_area_force_load_assets', true) ) {
...
if ( ! function_exists( 'etheme_woocommerce_widget_shopping_cart_proceed_to_checkout' ) ) {
...
if ( ! function_exists( 'etheme_get_single_product_class' ) ) {
	function etheme_get_single_product_class( $layout ) {
		...
		if ( $layout != 'large' ) {
			if ( $layout != 'fixed' && etheme_get_option( 'fixed_images', 0 ) ) {
				$classes['class'][] = 'product-fixed-images';
			} else if ( $layout == 'fixed' || etheme_get_option( 'fixed_content', 0 ) ) {
...
if ( ! function_exists( 'etheme_360_view_block' ) ) {
	function etheme_360_view_block() {
		...
		<?php
		echo '<div id="product-360-popup" class="product-360-popup mfp-hide">';
		    $slider->show();
		echo '</div>';
	}
}
...
if ( ! function_exists( 'etheme_catalog_setup' ) ) {
	function etheme_catalog_setup($force_apply_filters = false) {
		...
		if ( etheme_is_catalog() ) {
			...
			add_filter('xstore_wishlist_page_content', function ($elements) {
			    if ( in_array('price', $elements) ) {
			        unset($elements[array_search('price', $elements)]);
			    }
			    return $elements;
			});
			add_filter('xstore_compare_page_content', function ($elements) {
			    if ( in_array('price', $elements) ) {
			        unset($elements[array_search('price', $elements)]);
			    }
			    return $elements;
		...
		// **********************************************************************//
		$products_per_page = isset( $_REQUEST['et_per_page'] ) ? $_REQUEST['et_per_page'] : apply_filters('etheme_products_per_page', etheme_get_option( 'products_per_page', 12 ));
...
if ( ! function_exists( 'etheme_add_to_cart_catalog_button' ) ) {
	function etheme_add_to_cart_catalog_button( $sprintf, $product, $args ) {
		return sprintf( '<a rel="nofollow" href="%s" class="button login-to-buy-product show-product">%s</a>',
...
add_action( 'wp_ajax_etheme_check_product_in_cart', 'etheme_check_product_in_cart' );
add_action( 'wp_ajax_nopriv_etheme_check_product_in_cart', 'etheme_check_product_in_cart' );

if ( !function_exists('etheme_check_product_in_cart') ) {
    function etheme_check_product_in_cart() {
        $product_ids = [];
...
if ( ! function_exists( 'etheme_product_quick_view' ) ) {
	function etheme_product_quick_view() {
		...
		if ( $the_query->have_posts() ) {
            $canvas_type = etheme_get_option( 'quick_view_content_type', 'popup' ) == 'off_canvas';
			...
			echo esc_html__('No posts were found!', 'xstore');
		}
		die();
	}
}
...
add_action( 'wp_ajax_etheme_update_cart_item_quantity_by_id', 'etheme_update_cart_item_quantity_by_id' );
add_action( 'wp_ajax_nopriv_etheme_update_cart_item_quantity_by_id', 'etheme_update_cart_item_quantity_by_id' );
if ( ! function_exists( 'etheme_update_cart_item_quantity_by_id' ) ) {
	function etheme_update_cart_item_quantity_by_id() {
		$id = intval( $_REQUEST['item_id'] );
        $quantity = intval( $_REQUEST['qty'] );
//        $product    = isset( $_POST['item_id'] ) ? wc_get_product( absint( $_POST['item_id'] ) ) : false;
        ...
        foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
            ...
            if( in_array( $cart_item['data']->get_id(), $specific_ids ) && $cart_item['quantity'] != $new_qty ){
...
if ( ! function_exists( 'etheme_size_guide' ) ) {
	function etheme_size_guide($custom_settings = array()) {
		...
		$settings = wp_parse_args($custom_settings, array(
		   'image' => $global_guide,
		   'type' => $local_type != '' ? $local_type : $global_type,
		   'file' => $size_file,
		   'icon' => false,
		   'icon_position' => 'left',
		   'title' => esc_html__( 'Sizing guide', 'xstore' ),
		   'elementor_widget' => false,
		));

		if ( $local_type != '' )
		    $settings['type'] = $local_type;
		...
		if ( $local_guide ) {
			$image = $local_guide;
		} elseif ( $settings['type'] == 'popup' && isset($settings['image']['url']) ) {
			$image = $settings['image']['url'];
		} elseif ( $settings['type'] == 'download_button' && ! empty( $settings['file'] ) ) {
			$image = $settings['file'];
		} else {
			$image = '';
		}
		...
		if ( ! empty( $image ) ) : ?>
			...
			if ( $settings['type'] == 'popup' ) {
			    // Elementor has own functions for opening popup
			    if ( !$settings['elementor_widget'] )
				    $attr[] = 'rel="lightbox"';
			} else {
			...
			if ($settings['elementor_widget']) {
			    $settings['title'] = '<span class="button-text">'.$settings['title'].'</span>';
			    if ( $settings['icon'] )
			        $settings['icon'] = '<span>'.$settings['icon'].'</span>';
			    $attr[] = 'class="elementor-button"';
            }
			?>
            <div class="size-guide">
                <?php echo '<a ' . implode( ' ', $attr ) . '>' .
                    ($settings['icon'] && $settings['icon_position'] == 'left' ? $settings['icon'] : '' ) .
                        $settings['title'] .
                    ($settings['icon'] && $settings['icon_position'] == 'right' ? $settings['icon'] : '' ) .
                '</a>'; ?>
            </div>
		<?php endif;
	}
		...
		if (
			...
			if ( ! is_wp_error( $primary ) ) {
				...
				if ( ! is_wp_error( $term_link ) ) {
					if ( $single ) {
					    // It's done to avoid translations error
					    $c_text = _nx( 'Category:', 'Categories:', 1, 'Category: / Categories: text', 'xstore' );
						$html .= '<span class="posted_in">' . $c_text . '<a href="' . esc_url( $term_link ) . '">' . $primary->name . '</a></span>';
					} else {
...
if ( ! function_exists( 'etheme_grid_list_switcher' ) ) {
	function etheme_grid_list_switcher() {
        ...
        if ( !apply_filters('etheme_grid_list_switcher_enabled', true) ) return;
...
if ( ! function_exists( 'etheme_get_view_mode' ) ) {
	function etheme_get_view_mode() {
		...
		if ( get_query_var('et_is_customize_preview', false) ) {
			...
			if ( in_array($mode, array('list_grid', 'list') ) ) {
				$current = 'list';
			}
		...
		if ( class_exists( 'WC_Session_Handler' ) && ! is_admin() ) {
			...
			if ( isset( $_REQUEST['view_mode'] ) ) {
				...
				$current = ( $s );
			} elseif ( in_array($mode, array('list_grid', 'list') ) ) {
				$current = 'list';
			}
		}
...
if ( !function_exists('etheme_get_current_page_url')) {
	function etheme_get_current_page_url() {
        ...
        // prevent some unique cases of fatal errors
        if ( is_wp_error($link) )
            $link = home_url();
	    ...
	    foreach ($query_atts as $query_attr) {
	        if ( isset( $_GET[$query_attr] ) )
                $link = add_query_arg( $query_attr, wc_clean( wp_unslash($_GET[$query_attr]) ), $link );
	    }
...
if ( ! function_exists( 'etheme_products_per_page_select' ) ) {
	function etheme_products_per_page_select() {
        if ( !apply_filters('etheme_products_per_page_select_enabled', true) ) return;
		...
		if ( !apply_filters('etheme_elementor_theme_builder', false) && is_active_sidebar( 'shop-filters-sidebar' ) ) {
        ...
        if ( apply_filters('etheme_products_per_page_select_heading', true ) ) {
		...
		foreach ( $_GET as $key => $val ) {
			if (
			        in_array($key, array('et_per_page', 'submit')) || (
                    'et_ajax' === $key &&
...
//			global $etheme_mini_cart_global;
			$cart_content_linked_products_dt = apply_filters('etheme_mini_cart_linked_products', etheme_get_option('cart_content_linked_products_et-desktop', false));
			$cart_content_linked_products_mob = apply_filters('etheme_mini_cart_linked_products', etheme_get_option('cart_content_linked_products_et-mobile', false));
			$cart_content_linked_products_type = apply_filters('etheme_mini_cart_linked_products_type', etheme_get_option('cart_content_linked_products_type_et-desktop', 'upsell'));
			$is_mobile = (wp_doing_ajax() ? wp_is_mobile() : get_query_var('is_mobile', false));
            ...
            $show_cart_content_linked_products = apply_filters('etheme_mini_cart_linked_products_force_display', $show_cart_content_linked_products);
            ...
            <ul class="cart-widget-products clearfix">
				...
				foreach ( $cart as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						$product_remove_icon = apply_filters( 'woocommerce_cart_item_remove_icon_html', '<i class="et-icon et-delete et-remove-type1"></i><i class="et-trash-wrap et-remove-type2"><img src="' . ETHEME_BASE_URI . 'theme/assets/images/trash-bin.gif' . '" alt="'. esc_attr( sprintf( __( 'Remove %s from cart', 'xstore' ), $product_name ) ) .'"></i>' ); ?>
                        <li class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>"
							...
							echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
							    ...
							    esc_attr( sprintf( __( 'Remove %s from cart', 'xstore' ), wp_strip_all_tags( $product_name ) ) ),
								esc_attr( $product_id ),
								esc_attr( $cart_item_key ),
								esc_attr( $_product->get_sku() ),
                                /* translators: %s is the product name */
							    esc_attr( sprintf( __( '&ldquo;%s&rdquo; has been removed from your cart', 'xstore' ), wp_strip_all_tags( $product_name ) ) ),
								$product_remove_icon
							), $cart_item_key );
							?>
                            ...
                            <div class="product-item-right">
                                ...
                                <div class="descr-box">
									...
									<?php
									$product_quantity_input = ( etheme_get_option('cart_content_quantity_input_et-desktop', false) && !$is_mobile ) || ( etheme_get_option('cart_content_quantity_input_et-mobile', false) && $is_mobile );
                                    $product_quantity_input = apply_filters('etheme_mini_cart_quantity_input', $product_quantity_input);
		 							if ( ! $_product->is_sold_individually() && $_product->is_purchasable() && $product_quantity_input ) {
										...
										add_action( 'woocommerce_before_quantity_input_field', 'et_quantity_minus_icon' );
										add_action( 'woocommerce_after_quantity_input_field', 'et_quantity_plus_icon' );
										echo '<div class="quantity-wrapper clearfix size-sm">';
...
if ( ! function_exists( 'etheme_get_external_video' ) ) {
	function etheme_get_external_video( $post_id ) {
		...
		return get_post_meta( $post_id, '_product_video_code', true );
	}
}

if ( ! function_exists( 'etheme_get_external_video_placeholder' ) ) {
	function etheme_get_external_video_placeholder( $iframe ) {
		if (str_contains($iframe, 'youtube.com')) {
            preg_match('/youtube\.com\/embed\/([a-zA-Z0-9_-]+)/', $iframe, $matches);

            if (isset($matches[1])) {
                return 'https://img.youtube.com/vi/'.$matches[1].'/maxresdefault.jpg';
            }
        } elseif(str_contains($iframe, 'vimeo.com')) {
		    preg_match('/vimeo\.com\/video\/([0-9]+)/', $iframe, $matches);

            if (isset($matches[1])) {
                $vimeoUrl = 'https://vimeo.com/api/oembed.json?url=https://vimeo.com/'.$matches[1];
                $response = wp_remote_get($vimeoUrl);

                if (!is_wp_error($response)) {
                    $body = wp_remote_retrieve_body($response);
                    $vimeoData = json_decode($body, true);

                    if (isset($vimeoData['thumbnail_url'])) {
...
if ( ! function_exists( 'etheme_available_variation_gallery' ) ):
	function etheme_available_variation_gallery( $available_variation, $variationProductObject, $variation ) {
        ...
        if ( !$has_variation_gallery_images ) {
			$variation_id = apply_filters( 'wpml_object_id', $variation_id, 'product_variation', false, apply_filters('wpml_default_language', NULL ));
			$has_variation_gallery_images = (bool) get_post_meta( $variation_id, $variation_gallery_images_meta_key, true );
		}
    ...
    <script type="text/html" id="tmpl-et-variation-gallery-slider-template">
        <div class="swiper-slide">
            <div class="woocommerce-product-gallery__image">
                <a data-elementor-open-lightbox="<?php echo !apply_filters('etheme_single_product_variation_gallery_lightbox', false) ? 'no' : 'yes' ?>" data-elementor-lightbox-slideshow="all-slide-<?php echo $post->ID; ?>" href="{{data.url}}" data-large="{{data.full_src}}" data-width="{{data.full_src_w}}" data-height="{{data.full_src_h}}" data-index="{{data.index}}" class="woocommerce-main-image <# if (data.index < 1) { #> pswp-main-image <# } #> zoom">
...
function thumbnail_template_js() {
    ...
    <script type="text/html" id="tmpl-et-variation-gallery-thumbnail-template">
        <li class="<?php echo get_query_var('etheme_single_product_vertical_slider', false) ? 'slick-slide' : 'swiper-slide'; ?> thumbnail-item" <?php echo get_query_var('etheme_single_product_vertical_slider', false) ? 'style="width: 100%;"' : ''; ?>>
            <span data-elementor-open-lightbox="no" data-elementor-lightbox-slideshow="all-slide-<?php echo $post->ID; ?>" href="{{data.url}}" data-small="{{data.src}}" data-large="{{data.full_src}}" data-width="{{data.full_src_w}}" data-height="{{data.full_src_h}}" class="pswp-additional pointer zoom" title="{{data.a_title}}">
...
function et_add_action_to_multi_currency_ajax( $ajax_actions ) {
	$ajax_actions[] = 'etheme_product_quick_view'; // Add a AJAX action to the array
	$ajax_actions[] = 'etheme_added_to_cart_popup';
    $ajax_actions[] = 'etheme_elementor_menu_cart_fragments';
	return $ajax_actions;
}
...
if ( !function_exists('etheme_woocommerce_before_add_to_cart_quantity_with_type') ) {
    function etheme_woocommerce_before_add_to_cart_quantity_with_type() {
        ?>
        <div class="quantity-wrapper type-{{quantity_type}}" data-label="<?php echo esc_html__( 'Quantity:', 'xstore' ); ?>">
...
if ( !function_exists('etheme_woocommerce_after_add_to_cart_quantity_with_type') ) {
    function etheme_woocommerce_after_add_to_cart_quantity_with_type() {
        ?>
...
if ( !function_exists('etheme_woocommerce_woocommerce_quantity_input_class_duplicated') ) {
    function etheme_woocommerce_woocommerce_quantity_input_class_duplicated($classes) {
        $classes[] = 'qty-duplicated';
        return $classes;
    }
}
...
if (!function_exists('etheme_buy_now_btn')) {
	function etheme_buy_now_btn($type = '') {
		if ( !apply_filters('etheme_buy_now_enabled', etheme_get_option( 'buy_now_btn', 0 )) ) {
			return;
		}

		$btn_text = apply_filters( 'etheme_buy_now_button_text', esc_html__( 'Buy now', 'xstore' ) );

		global $product;
		if ( !in_array($product->get_type(), array('external')) ) {
		    if ( apply_filters('etheme_buy_now_or_separator', true)) : ?>
            ...
            <?php if($type == 'variable-quick-view') : ?>
                <a
                    ...
                    <?php if ( apply_filters('etheme_elementor_theme_builder', false) ) : ?>
                        data-builder-element="true"
                    <?php endif; ?>
                    class="<?php echo implode(' ', apply_filters( 'etheme_buy_now_button_classes', array('et_product_variable-in-quick-view', 'product_type_variable', 'add_to_cart_button', 'et-st-disabled', 'et-single-buy-now', 'single_add_to_cart_button', 'button', 'alt'))); ?>"
                >
                    <?php echo apply_filters( 'etheme_buy_now_button_text', esc_html__( 'Select options', 'xstore' ) ); ?>
                </a>
			<?php else : ?>
                <button type="submit" data-quantity="" data-product_id="" class="<?php echo implode(' ', apply_filters( 'etheme_buy_now_button_classes', array('et-single-buy-now', 'single_add_to_cart_button', 'button', 'alt'))); ?>"<?php if ( apply_filters('etheme_elementor_theme_builder', false) ) : ?>
                        data-builder-element="true"
                    <?php endif; ?>><?php echo esc_html($btn_text); ?></button>
			<?php endif; ?>
		<?php }
	}
}
...
function etheme_show_single_stock(){
    if ( !apply_filters('etheme_show_single_stock', etheme_get_option( 'show_single_stock', 0 )) ) {
		return;
	}
	global $product;
	if (
	    ...
	    if ( get_theme_mod('advanced_stock_status', false) && in_array('single_product', (array)etheme_get_option( 'advanced_stock_locations', array('single_product', 'quick_view') )) && 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
	        echo et_product_stock_line($product, '<p class="et_stock et_in-stock stock in-stock step-1">'.wc_format_stock_for_display($product).'</p>');
	    }
	    else {
		    echo '<p class="et_stock et_in-stock stock in-stock step-1">'.wc_format_stock_for_display($product).'</p>';
		}
	}
}
...
function etheme_show_single_variation_stock($return,$variation){
	...
	if(empty($return['availability_html'])){
		$return['availability_html'] = '<p class="et_stock et_in-stock stock in-stock step-1">'.wc_format_stock_for_display($variation).'</p>';
	}
	if ( get_theme_mod('advanced_stock_status', false) && in_array('single_product', (array)etheme_get_option( 'advanced_stock_locations', array('single_product', 'quick_view') )) && 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
...
function et_add_custom_field_variation_data( $variations ) {
    ...
    if ( $date_end ) {
        ...
        $variations['_sale_price_end_gmt'] = strtotime(
                get_gmt_from_date(date('Y-m-d', $date_end) . ' '.
        ((isset( $time_end[0] ) && $time_end[0] != 'Array' && $time_end[0] > 0 ) ? $time_end[0] : '00').':'.
        ((isset( $time_end[1] ) ? $time_end[1] : '00').':00')));
    }
...
function et_custom_price_html( $price, $product ) {
	...
	if ( $is_catalog && etheme_get_option('just_catalog_price', 0) ) {
        if ( $etheme_ltv_price != '' ) {
            return etheme_get_option('ltv_price_button', false) ?
             sprintf( '<a rel="nofollow" href="%s" class="button login-to-buy-product show-product">%s</a>',
...
//			$outofstock .= " AND p.ID IN ( SELECT $wpdb->term_relationships.object_id  FROM $wpdb->term_relationships WHERE term_taxonomy_id  IN (" . get_queried_object()->term_id . ") )";
	...
	if ( $variation_product_widgets_recount ) {
		...
		add_filter('etheme_cats_widget_count', 'etheme_brands_widget_count_filter', 10, 3);
						...
						if ( $attribute_to_limit_parse ) {
                            ...
                            if (array_key_exists('pa_'.$attribute_to_limit, $attributes)) {
                                if ( !empty($attributes['pa_'.$attribute_to_limit]) ) {
                                    if ( $hide_outofstock && (!$variation_product || ! $variation_product->is_in_stock() || !$variation_product->variation_is_visible()) ) {
                                    }
                                    else
...
if ( !function_exists('etheme_product_empty_rating') ) {
    function etheme_product_empty_rating($html, $rating, $count) {
        if ( $rating > 0) return $html;
        /* translators: %s: rating */
		$label = sprintf( __( 'Rated %s out of 5', 'xstore' ), $rating );
...
if ( get_theme_mod('cats_subcategories_line_separated', false) ) {
    add_filter('dynamic_sidebar_params', function ($params) {
        if (strpos($params[0]['widget_id'], 'woocommerce_product_categories' ) === false )
            return $params;

        if ( isset($params[0]['before_widget']) ) {
            $params[0]['before_widget'] = str_replace(
                'class="', 'class="subcategories-line-separated ', $params[0]['before_widget']);
        }
        return $params;
    }, 999);
}
...
add_filter('woocommerce_before_widget_product_review_list', function ($html){
	etheme_enqueue_style( 'cart-widget' );
	return $html;
});
...
if (!function_exists('et_get_active_brand_ids')){
	function et_get_active_brand_ids($filter_brand, $taxonomy = 'brand'){
...
function etheme_set_fake_live_viewing_count($id) {
    ...
    return do_shortcode(str_replace(array('{eye}', '{count}'), array('<i class="et-icon et-view"></i>', $average_count), $xstore_sales_booster_settings_fake_live_viewing['message']));
...
if ( !function_exists('etheme_get_fake_product_sales_count') ) {
    function etheme_get_fake_product_sales_count($id) {
        ...
        return $average_count ? do_shortcode(str_replace(
...
add_filter('woocommerce_structured_data_product', function ($markup, $product) {
	if ( !get_query_var('et_brands', true) || etheme_xstore_plugin_notice() ) {
		return $markup;
	}
...
if ( class_exists('Etheme_Sales_Booster_Progress_Bar') ) {
    $sale_booster_progress_bar = Etheme_Sales_Booster_Progress_Bar::get_instance();
    $sale_booster_progress_bar->init();
}

if ( class_exists('Etheme_Sales_Booster_Estimated_Delivery') ) {
    $sale_booster_estimated_delivery = new Etheme_Sales_Booster_Estimated_Delivery();
    $sale_booster_estimated_delivery->init();
}

if ( class_exists('Etheme_Sales_Booster_Safe_Checkout') ) {
    $sale_booster_safe_checkout = Etheme_Sales_Booster_Safe_Checkout::get_instance();
    $sale_booster_safe_checkout->init();
}

if ( class_exists('Etheme_Sales_Booster_Quantity_Discounts') ) {
    $sale_booster_quantity_discounts = Etheme_Sales_Booster_Quantity_Discounts::get_instance();
    $sale_booster_quantity_discounts->init();
}

if ( class_exists('Etheme_WooCoommerce_Account') ) {
    $sale_booster_account = Etheme_WooCoommerce_Account::get_instance();
    $sale_booster_account->init();
}

// ADDED: Compatibility with WooCommerce PayPal Payments plugin ( https://wordpress.org/plugins/woocommerce-paypal-payments/ )
add_filter( 'woocommerce_paypal_payments_single_product_renderer_hook', function($filter){
	return get_query_var( 'etheme_single_product_builder', false ) ? 'etheme_product_single_wishlist' : $filter;
	// return 'etheme_woocommerce_template_single_add_to_cart';
} );

// ADDED: Compatibility with YITH WooCommerce Dynamic Pricing & Discounts Premium ( https://yithemes.com/themes/plugins/yith-woocommerce-dynamic-pricing-and-discounts/ )
add_filter('ywdpd_column_product_info_class', function($class) {
    return get_query_var( 'etheme_single_product_builder', false ) ? '.single-product-builder' : '.single-product .product-content';
});

// Get drafted products ids
if ( !function_exists('etheme_get_draft_products_ids') ) {
    function etheme_get_draft_products_ids() {
        $ids = get_transient( 'etheme_get_draft_products_ids', false );
        if ($ids === false){
            $args = array(
                'post_type'      => 'product',
            ...
            $ids = get_posts($args);
            set_transient( 'etheme_get_draft_products_ids', $ids, DAY_IN_SECONDS );
        }
		return $ids;
    }
}
...
if(etheme_get_option( 'hide_cats_with_hidden_prods', 0 )){
    add_filter('get_terms', 'et_hide_empty_categories', 10, 3);
    add_filter( 'woocommerce_product_categories_widget_args', 'et_filter_product_categories_widget_args', 10, 1 );
}

if(etheme_get_option( 'outofstock_products_at_end', 0 )){
...
// Filter product categories widget args
function et_filter_product_categories_widget_args( $args ) {
    $args['hide_empty'] = true; // This hides categories without posts by default, but we need to ensure it considers visibility.
    return $args;
}

// Get categories with at least one visible product
function et_hide_empty_categories($terms, $taxonomies, $args) {
    $new_terms = array();
    if (in_array('product_cat', (array) $taxonomies) && !is_admin() && is_shop()) {
        foreach ($terms as $key => $term) {
            if (et_category_has_visible_products($term)) {
                $new_terms[] = $term;
            }
        }
        $terms = $new_terms;
...
// Check category has visible products
function et_category_has_visible_products($term) {
    $args = array(
        'post_type'      => 'product',
        'post_status'    => 'publish',
        'posts_per_page' => -1,
