	    ...
	    <?php if( $instagram_API['business']['display'] ) : ?>
            <p style="<?php echo (!$instagram_API['business']['enabled']) ? 'pointer-events: none; opacity: 0.5;' : ''; ?>">
                <label for="business">
                    ...
                    <b><?php esc_html_e('Business','xstore'); ?></b> <?php esc_html_e('(Based on Graph API. Allows you to show media from the Instagram account of the Business type and media by hashtags. Recommended for owners of the Business Instagram accounts) ', 'xstore'); ?>
                </label>
            </p>
	    <?php endif; ?>
