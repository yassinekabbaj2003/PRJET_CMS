...
class Nav_Menu_Images {
	...
	public function init() {
		if ( ! etheme_get_option('et_menu_options', 1) ) {
			return;
		}
		...
		require_once dirname( __FILE__ ) . '/inc/field-types.php';
	...
	public function load_textdomain() {
		...
		if ( ! is_textdomain_loaded( 'nmi' ) )
			load_theme_textdomain( 'nmi', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
	...
	public function menu_item_content( $content, $item_id ) {
		if ( has_post_thumbnail( $item_id ) )
			// etheme ajaxify feature makes dublication of nav images on menu item because of filter it uses the_title
			if ( defined('DOING_ETHEME_AJAXIFY') ) return $content;
	...
	public function action_links( $links ) {
		...
		$links['donate'] = '<a href="">' . esc_html__( 'Donate', 'xstore' ) . '</a>';
		return $links;
	}
}
