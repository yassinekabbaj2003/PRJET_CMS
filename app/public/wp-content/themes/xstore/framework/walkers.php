	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $depth == 0 ) {
			...
			if ( $design == 'posts-subcategories' && apply_filters( 'menu_item_with_sublists', true ) ) {
                ...
                // force load style in cases when Menu displayed from Elementor
		...
		if ( $design != '' ) {
            ...
            $should_load_mega_menu_assets = in_array($design, array('item-design-mega-menu', 'item-design-posts-subcategories'));
            if ( $should_load_mega_menu_assets ) {
                do_action('menu_item_mega_menu_assets_load');
            }
			if ( apply_filters( 'menu_item_mega_menu', $should_load_mega_menu_assets ) ) {
        ...
        $nav_menu_css_classes = apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args, $depth);
		$class_names = join( ' ', $nav_menu_css_classes);
...
//		$id = strlen( $id ) ? ' id="' . esc_attr( $id ) . '"' : '';
		...
		if ( $use_img == 'img' || ( $depth > 0 && get_post_thumbnail_id( $item_id, 'thumb' ) != '' ) ) {
            ...
            $link_classes[] = 'type-img';
            $link_classes[] = 'position-'.$bg_position;
		}
        $link_classes = apply_filters('etheme_menu_link_classes', $link_classes, array_filter($classes), $depth);
...
//		}
		...
		$this->et_enque_styles( $item_id, $depth, $tooltip );
        ...
        $item_output_inner = $icon;
        $item_output_inner .= $args->link_before;
        $item_title = apply_filters( 'the_title', $item->title, $item->ID );

		if ( !($depth < 3 && $disable_titles == 1) ) {
            $item_output_inner .= $item_title;
		}
        $item_output_inner .= $args->link_after;
        $item_output_inner .= $description;
        $item_output_inner .= $tooltip;
        $item_output_inner .= $label_text;
...
//				$item_output .= '<img src="' . $post_thumbnail_url . '" alt="menu-item-img">';
                $item_output_inner .= wp_get_attachment_image( $post_thumbnail, 'thumb');
			}
		}

        $item_output .= apply_filters('etheme_nav_menu_item_inner', $item_output_inner, $nav_menu_css_classes, $depth);
		$item_output .= '</a>';
	...
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
        ...
        if ( has_action('etheme_after_menu_item') ) :
            ob_start();
                do_action( 'etheme_after_menu_item', $depth );
            $output .= ob_get_clean();
        endif;
	}
	
	function et_enque_styles( $item_id, $depth, $has_tooltip = false ) {
		$visibility = get_post_meta( $item_id, '_menu-item-item_visibility', true );
		if ( ( $visibility == 'logged' && ! ( get_query_var( 'et_is-loggedin', false) ) ) || ( $visibility == 'unlogged' && get_query_var( 'et_is-loggedin', false) ) ) {
			return;
		}
		...
		if ( $use_img != 'img' && apply_filters('menu_item_load_background_style', true, $has_tooltip) ) {
			if ( $bg_position != '' ) {
...
//		$post_thumbnail_url = wp_get_attachment_image_src( $post_thumbnail, 'large' );
        ...
        $output             .= wp_get_attachment_image( $post_thumbnail, 'large');
//		$output             .= '<img src="' . $post_thumbnail_url[0] . '" width="' . $post_thumbnail_url[1] . '" height="' . $post_thumbnail_url[2] . '" />';
	...
	function getPostsByCategory( $cat = false ) {
		...
		if ( ! empty( $posts ) ) {
            ...
            // added !defined('DOING_ETHEME_AJAXIFY') condition for correct work of such menu in Elementor Header editor
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX && !defined('DOING_ETHEME_AJAXIFY') ) {
				echo ob_get_clean();
//                if ( !defined('DOING_ETHEME_AJAXIFY'))
//    				die(); // creates server error on Elementor editor
			} else {
				return ob_get_clean();
			}
		}
