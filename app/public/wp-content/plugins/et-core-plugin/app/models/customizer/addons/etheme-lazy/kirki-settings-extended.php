...
class Kirki_Settings_Extended {
	 ...
	 * @var array
	 */
	protected $setting_instance = array();
	 ...
	 */
	public function __construct( $args = array(), $setting_id = null ) {
		global $wp_customize;
		...
		// Add the settings.
		$this->add_settings( $args, $setting_id );
	}
	 ...
	 * @param array $args The field definition as sanitized in Kirki_Field.
	 */
	function add_settings( $args = array(), $setting_id = null ) {
		...
		if ( isset( $args['settings'] ) && is_array( $args['settings'] ) ) {
			...
			foreach ( $args['settings'] as $key => $value ) {
				...
				$this->add_setting( $classname, $value, $default, $args['option_type'], $args['capability'], $args['transport'], $args['sanitize_callback'], $setting_id );
			}
		}

		$this->add_setting( $classname, $args['settings'], $args['default'], $args['option_type'], $args['capability'], $args['transport'], $args['sanitize_callback'], $setting_id );
	}
	 ...
	 */
	function add_setting( $classname, $setting, $default, $type, $capability, $transport, $sanitize_callback, $setting_id ) {
		...
		$setting_instance = new $classname( $this->wp_customize,$setting_id, $args);

		$this->wp_customize->add_setting( $setting_instance );

		$this->setting_instance = $setting_instance;
	}
	...
	function json() {
		return $this->setting_instance->json();
	}
}
