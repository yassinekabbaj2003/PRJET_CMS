(function($, api) {
	...
	api.sectionConstructor['kirki-lazy'] = api.sectionConstructor.default.extend({
		...
		finishLoading: function(sectionID){
			...
			setTimeout(function() {
				if(section.promise) {
					...
					if ( section.id == 'header_presets' || section.id == 'single_product_import_export_section') {
						$('#' + $('#accordion-section-' + section.id).attr('aria-owns')).find('.customize-control-title-ghost').addClass('customize-control-title').removeClass('customize-control-title-ghost');
					}
		...
		et_add_options_devices : function(section) {
			...
			if ( section.id == 'customizer-user-preferences' ) {
				if ( $(window).width() <= 1200 )
					$(document).find('#customize-control-customizer_ui_width').addClass('hidden');
				// hide tooltips option because the device user comes from does not support hover so it will be forcefully
				// shown as visible descriptions on options
				if ( window.matchMedia && window.matchMedia('(hover: none)').matches )
					$(document).find('#customize-control-customizer_options_descriptions').addClass('hidden');
			}
		},
	...
	function kirkiTooltipAdd( controls ) {
		_.each( controls, function( control ) {
			...
			controlID = '#customize-control-' + control.id + ' > label .customize-control-title,' +
				'#customize-control-' + control.id + ' > .kirki-input-container > label .customize-control-title,' +
				'#customize-control-' + control.id + '.customize-control-kirki-multicolor > .customize-control-title,' +
				'#customize-control-' + control.id + '.customize-control-kirki-multicheck > .customize-control-title,'+
				'#customize-control-' + control.id + '.customize-control-upload > .customize-control-title,' +
				'#customize-control-' + control.id + '.customize-control-kirki-radio-buttonset > .customize-control-title';
			content   = '<span class="tooltip-content" data-setting="' + control.id + '">' + control.params.tooltip.content + '</span>';
