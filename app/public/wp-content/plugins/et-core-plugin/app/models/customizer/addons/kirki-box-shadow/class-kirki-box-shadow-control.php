...
class Kirki_Box_Shadow_Control extends WP_Customize_Control {
	...
	protected function content_template() {
		...
		<div class="kirki-input-container">
			<div class="preview-wrapper">
				<div class="preview-object">
					...
					<span class="screen-reader-text"><?php esc_html_e( 'Click and drag to adjust horizontal & vertical length', 'xstore-core' ); ?></span>
		...
		<div class="kirki-input-container">
			...
			<label for="{{ data.id }}-inset"><?php esc_html_e( 'Inset', 'xstore-core' ); ?></label>
		</div>

		<div class="kirki-input-container">
			<label for="{{ data.id }}-blur-radius"><?php esc_html_e( 'Blur Radius', 'xstore-core' ); ?></label>
			<input class="blur-radius" type="range" min="0" max="50" step="1" value="0" id="{{ data.id }}-blur-radius" data-context="blur-radius"/>
		</div>

		<div class="kirki-input-container">
			<label for="{{ data.id }}-spread-radius"><?php esc_html_e( 'Spread Radius', 'xstore-core' ); ?></label>
			<input class="spread-radius" type="range" min="0" max="50" step="1" value="0" id="{{ data.id }}-spread-radius" data-context="spread-radius"/>
		</div>

		<div class="kirki-input-container">
			<label for="{{ data.id }}-color"><?php esc_html_e( 'Shadow Color', 'xstore-core' ); ?></label>
			<input id="{{ data.id }}-color" type="text" data-default-color="{{ data.default }}" value="{{ data.value.color }}" class="kirki-color-control" data-id="{{ data.id }}" data-alpha="true"/>
		</div>
