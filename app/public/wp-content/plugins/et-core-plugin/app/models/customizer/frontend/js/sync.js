(function (api) {
     ...
     * @param output
     */
    api.previewerSync.registerStyleOutput = function (id, options) {
        if ('undefined' == typeof( options.params.output ) || null == options.params.output ) {
            return;
        }
        if (options.params.output.length) {
            _.each(options.params.output, function (style) {
                style.default = options.params.default;
            });

            var param = {
                id: id,
                output: options.params.output,
                options: options
            };
