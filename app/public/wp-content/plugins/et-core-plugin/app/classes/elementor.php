	...
	public static function get_post_pages( $array = array('post'), $by_slug = false ) {
		...
		if ( $the_query->have_posts() ) : 
			while ( $the_query->have_posts() ) : 
				...
				$id = get_the_ID();
                if ( $by_slug )
                    $list[get_post_field('post_name', $id)] = get_the_title() . ' (id - ' . $id . ')';
                else
				    $list[$id] = get_the_title() . ' (id - ' . $id . ')';
			endwhile;
			wp_reset_postdata();
...
//				'mobile_default' => 2,
    ...
    public static function get_post_template( $term = 'page' ) {
        $posts = get_posts(
            [
    ...
    public static function get_saved_content( $term = 'section', $by_slug = true ) {
        $saved_contents = self::get_post_template( $term );
