...
class Share extends Shortcodes {
	...
	public static function share_shortcode( $atts, $content = null ) {
		...
		$tooltip_class = '';
		if($tooltip) {
			$tooltip_class = 'title-tooltip mtips mtips-top';
		}
		if ( $copy_click )
		...
		if($twitter == 1) {
			...
			<i class="et-icon et-twitter"></i>'.
                ($tooltip ? '<span class="mt-mes">'.__('Twitter', 'xstore-core').'</span>' : '').
			'</a>
			</li>
			';
		}
		...
		if($facebook == 1) {
			...
			<i class="et-icon et-facebook"></i>'.
                ($tooltip ? '<span class="mt-mes">'.__('Facebook', 'xstore-core').'</span>' : '').
                '</a>
			</li>
			';
		}
		...
		if($vk == 1) {
			...
			<i class="et-icon et-vk"></i>'.
                ($tooltip ? '<span class="mt-mes">'.__('VK', 'xstore-core').'</span>' : '').
                '</a>
			</li>
			';
		}
		...
		if($pinterest == 1) {
			...
			<i class="et-icon et-pinterest"></i>'.
                ($tooltip ? '<span class="mt-mes">'.__('Pinterest', 'xstore-core').'</span>' : '').
                '</a>
			</li>
			';
		}
		...
		if($mail == 1) {
			...
			<i class="et-icon et-message"></i>'.
                ($tooltip ? '<span class="mt-mes">'.__('Mail to friend', 'xstore-core').'</span>' : '').
                '</a>
			</li>
			';
		}
		...
		if($linkedin == 1) {
			...
			<i class="et-icon et-linkedin"></i>'.
                ($tooltip ? '<span class="mt-mes">'.__('Linkedin', 'xstore-core').'</span>' : '').
                '</a>
			</li>
			';
		}
		...
		if($whatsapp == 1) {
            ...
            ($tooltip ? '<span class="mt-mes">'.__('Whatsapp', 'xstore-core').'</span>' : '').
            '</a>
			</li>
			';
		}
		...
		if($skype == 1) {
			$html .= '
			<li>
			<a href="https://web.skype.com/share?url='.$permalink.'" class="'.$tooltip_class.'" title="'.__('skype', 'xstore-core').'" target="_blank" rel="noopener">
			<i class="et-icon et-skype"></i>'.
            ($tooltip ? '<span class="mt-mes">'.__('Skype', 'xstore-core').'</span>' : '').
            '</a>
			</li>
			';
		}
