...
class Import extends Base_Controller {
	...
	private function add_widget( $sidebar, $widget, $options = array() ) {
		...
		$widget_content[ $this->widgets_counter ] = (array) $options;
	...
	public function import_custom_fonts(){
		...
		foreach ($new_fonts as $key => $value) {
			...
			$content      = file_get_contents( $remote_file, false, stream_context_create($arrContextOptions) );
			$uploads      = wp_get_upload_dir();
...
//		$this->log_imported_data('multiples', $option);
	...
	public function default_woocommerce_pages(){
		$shop_page = $this->get_page_by_title('Shop ' . $this->version);
		$cart_page = $this->get_page_by_title('Cart ' . $this->version);
		$checkout_page = $this->get_page_by_title('Checkout ' . $this->version);
		$my_account = $this->get_page_by_title('my account');

		if (!$cart_page){
			$cart_page = $this->get_page_by_title('cart');
		}

		if (!$checkout_page){
			$checkout_page = $this->get_page_by_title('checkout');
		}
		if (!$shop_page){
			$shop_page = $this->get_page_by_title('shop');
		}
...
//		update_option('et_theme_builder_wc_install_done', 'yes');
	...
	public function generate_remote_url($file){
		if (
			isset($this->versions[ $this->version ]['elementor_builders'])
			&& $this->versions[ $this->version ]['elementor_builders']
			&& in_array( $this->engine, array('elementor', 'elementor_builders'))
		){
