...
class CSS {
    ...
    public function add_custom_css_controls_section( $widget, $section_id, $args ){
        ...
        if ( !defined( 'ELEMENTOR_PRO_VERSION' ) )
            $this->remove_go_pro_custom_css_controls( $widget );
...
selector .child-element{ color: blue; }
    ...
    public function remove_go_pro_custom_css_controls( $controls_stack ) {
    ...
    public function add_custom_page_css( $page ) {
        if ( isset( $page ) && $page->get_id() > '' ) {
            ...
            if ( in_array( $page_options_post_type, array('page', 'revision', 'staticblocks', 'etheme_slides') ) ) {
