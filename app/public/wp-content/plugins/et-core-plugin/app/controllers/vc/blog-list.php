 ...
 */
class Blog_List extends VC {

	function hooks() {

		add_filter( 'vc_autocomplete_et_blog_list_include_callback', 'vc_include_field_search', 10, 1 );
		add_filter( 'vc_autocomplete_et_blog_list_include_render', 'vc_include_field_render', 10, 1 );

		add_filter( 'vc_autocomplete_et_blog_list_taxonomies_callback', 'vc_autocomplete_taxonomies_field_search', 10, 1 );
		add_filter( 'vc_autocomplete_et_blog_list_taxonomies_render', 'vc_autocomplete_taxonomies_field_render', 10, 1 );

		add_filter( 'vc_autocomplete_et_blog_list_exclude_filter_callback', 'vc_autocomplete_taxonomies_field_search', 10, 1 );
		add_filter( 'vc_autocomplete_et_blog_list_exclude_filter_render', 'vc_autocomplete_taxonomies_field_render', 10, 1 );

		add_filter( 'vc_autocomplete_et_blog_list_exclude_callback', 'vc_exclude_field_search', 10, 1 );
		add_filter( 'vc_autocomplete_et_blog_list_exclude_render', 'vc_exclude_field_render', 10, 1 );

		$this->register_vc_blog_list();
	}

	function register_vc_blog_list() {
		...
		$counter = 0;
		$params = array(
			'name' => 'Blog List / Chess',
			'base' => 'et_blog_list',
			'icon' => ETHEME_CODE_IMAGES . 'vc/Blog.png',
			'description' => esc_html__('Display posts in list / chess', 'xstore-core'),
			...
			'params' => array(
				...
				// 	'description' => esc_html__( 'Build custom query according to <a href="http://codex.wordpress.org/Function_Reference/query_posts">WordPress Codex</a>.', 'xstore-core' ),
				// 	'dependency' => array(
				// 		'element' => 'post_type',
				// 		'value' => 'custom',
				// 	),
				...
				array(
					...
					'value' => array(
						esc_html__('List', 'xstore-core') => 'small',
						esc_html__('Chess', 'xstore-core') => 'chess',
					),
					'images_value' => array(
						'small'   => ET_CORE_SHORTCODES_IMAGES . 'blog-list/List.svg',
						'chess'   => ET_CORE_SHORTCODES_IMAGES . 'blog-list/Chess.svg',
					),
					'et_tooltip' => true,
				),
				array(
