...
//		add_action( 'admin_init', array( $this, 'registerBackendJs' ), 999 );
        add_action( 'admin_print_scripts-post.php', array( $this, 'registerBackendJs' ) );
        add_action( 'admin_print_scripts-post-new.php', array( $this, 'registerBackendJs' ) );
	...
	public function registerBackendJs() {
		if ( !defined('WPB_VC_VERSION') ) return;
		wp_enqueue_script( 'xstore-vc-config', ET_CORE_URL . 'config/vc/js/config.min.js', array(), false, true );
	...
	public function vc_iconpicker_type_xstore_icons( $icons ) {
		$xstore_icons = array(
            ...
            array( 'et-icon et-viber' => 'et-viber' ),
            ...
            array( 'et-icon et-viber-o' => 'et-viber-o' ),
            ...
            array( 'et-icon et-wallet' => 'et-wallet' ),
            array( 'et-icon et-protect' => 'et-protect' ),
            array( 'et-icon et-money-bag' => 'et-money-bag' ),
            array( 'et-icon et-guarantee' => 'et-guarantee' ),
            array( 'et-icon et-trash' => 'et-trash' ),
            ...
            array( 'et-icon et-dot-inside' => 'et-dot-inside' ),
            array( 'et-icon et-dot' => 'et-dot' ),
		);
	...
	public function add_fonts($fonts) {
        ...
        if ( $etheme_typekit_fonts ) {
            ...
            foreach ( $typekit as $font ) {
                $et_fonts_list[] = '{"font_family":"' . $font . '"}';
            }
        }
	...
	public function etheme_product_image_render( $atts ) {
		...
		if ( $size == 'custom' ) {
			if (function_exists('etheme_get_size')){
				$size = etheme_get_size( $size_custom );
			} else {
				$size = $size_custom;
            }
...
/* **************************** */
	...
	function etheme_vc_shortcodes_strings() {
		return array(
			'category' => apply_filters('etheme_theme_label', 'XStore'),
			'heading'  => array(
