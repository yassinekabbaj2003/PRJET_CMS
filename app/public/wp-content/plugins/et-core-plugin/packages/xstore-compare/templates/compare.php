...
$compare_fields = apply_filters('xstore_compare_page_content', array_merge(get_theme_mod('xstore_compare_page_content', array(
    'action',
    'product',
    'title',
    'quantity',
...
if (!get_query_var('et_brands', true))
...
if (get_query_var('et_is_customize_preview', false)) {
    $element_options['form_attributes'][] = 'data-title="' . esc_html__('Compare', 'xstore-core') . '"';
...
//                                                echo '<label for="' . esc_attr( 'quantity-' . $grouped_product_child->get_id() ) . '" class="screen-reader-text">' . esc_html__( 'Buy one of this item', 'xstore-core' ) . '</label>';
//                                            } else {
...
//                                    }
                            ...
                            break;
                        case 'brand':
                            $brands = $product_parent ? wp_get_post_terms($product_parent_id, 'brand') : wp_get_post_terms($product_info['id'], 'brand');
                            ob_start();
...
//                            remove_filter('esc_html', array($instance, 'escape_text'), 10, 2);
                        ...
                        case 'rating':
                            ...
                            if ( $link_ratings > 0 || apply_filters('xstore_compare_product_rating_show_empty', false) ) {
                        ...
                        case 'stock_status':
                            ...
                            wc_get_template(
                                ...
                                array(
                                    ...
                                    'availability' => !$availability['availability'] ? esc_html__('In stock', 'xstore-core') : $availability['availability'],
                                )
                            );
                            $product_table_info[$products_count][$field] = ob_get_clean();
                            break;
...
//                                            $compare_fields[] = 'attribute_' . $product_attribute['label'];
                ...
                foreach ($compare_fields as $compare_field) {
                    ...
                    foreach ($product_table_info as $product_table_info_key) {
                        ...
                        $table_row_content .=
                            '<td class="xstore-compare-' . (str_replace(' ', '-', $compare_field) . (in_array($compare_field, array('quantity', 'stock_status')) ? ' mob-hide' : '')) . '" data-product_id="' . $product_table_info_key['id'] . '">' .
                    ...
                    if (!$table_row_content_empty && $table_row_content) {
                        echo '<tr class="xstore-compare-row-' . (str_replace(' ', '-', $compare_field) . (in_array($compare_field, array('quantity', 'stock_status')) ? ' mob-hide' : '')) . '">';
