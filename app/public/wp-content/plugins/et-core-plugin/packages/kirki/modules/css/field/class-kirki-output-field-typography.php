	...
	protected function process_output( $output, $value ) {
		...
		foreach ( $properties as $property ) {
			...
			if ( 'font-family' === $property ) {
				// added in 4.2.5 to add serif || sans-serif to font on frontend output
				if ( Kirki_Fonts::is_google_font( $value[ $property ]) ) {
					$google_fonts_array = Kirki_Fonts::get_google_fonts();
					if ( in_array($google_fonts_array[$value[ $property ]]['category'], array('serif', 'sans-serif'))) {
						$value[ $property ] .= ', '.$google_fonts_array[$value[ $property ]]['category'];
					}
				}
