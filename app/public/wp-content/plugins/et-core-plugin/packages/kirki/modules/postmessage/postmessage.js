		...
		fromOutput: function( output, value, controlType ) {
			...
			switch ( controlType ) {
				case 'kirki-typography':
					...
					// added in 4.2.5 to add serif || sans-serif to font on transport: auto
					var originFontfamily = value['font-family']; // keep origin font to load property
					var isGoogleFont = value['font-family'] && 'google' === kirkiParent.util.webfonts.getFontType( value['font-family'] );
					if ( isGoogleFont ) {
						let fontProperties = kirkiParent.util.webfonts.getFont( value['font-family'] );
						if ( !!fontProperties.category ) {
							if ( fontProperties.category == 'serif' || fontProperties.category == 'sans-serif')
								value['font-family'] += ', '+fontProperties.category;
						}
					}
					...
					if ( ! _.isUndefined( WebFont ) && isGoogleFont ) {

						// Calculate the googlefont params.
						googleFont = originFontfamily.replace( /\"/g, '&quot;' );
						if ( value.variant ) {
