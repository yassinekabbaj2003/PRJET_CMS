...
class Kirki_Control_Repeater extends Kirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->row_label = array(
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'xstore-core' ),
			'field' => false,
		);
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_html__( 'Add new %s', 'xstore-core' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'xstore-core' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	protected function render_content() {
				...
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?>
                    <?php if ( ! empty( $this->description ) ) : ?>
                        <span class="tooltip-wrapper">
                            <span class="tooltip-trigger" data-setting="<?php echo esc_attr($this->id); ?>">
                                <span class="dashicons dashicons-editor-help"></span>
                            </span>
                            <span class="tooltip-content" data-setting="<?php echo esc_attr($this->id); ?>">
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'xstore-core' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# if ( 'text' === field.type || 'url' === field.type || 'link' === field.type || 'email' === field.type || 'tel' === field.type || 'date' === field.type || 'number' === field.type ) { #>
								...
								<label>
                                    <# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}
                                        <# if ( field.description ) { #>
                                            <span class="tooltip-wrapper">
                                                <span class="tooltip-trigger" data-setting="{{{ field.id }}}">
                                                    <span class="dashicons dashicons-editor-help"></span>
                                                </span>
                                                <span class="tooltip-content" data-setting="{{{ field.id }}}">
                                                    {{{ field.description }}}
                                                </span>
                                            </span>
                                        <# } #>
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'xstore-core' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_html_e( 'No Image Selected', 'xstore-core' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'xstore-core' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'xstore-core' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'xstore-core' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change Image', 'xstore-core' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'xstore-core' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'xstore-core' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_html_e( 'No File Selected', 'xstore-core' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'xstore-core' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'xstore-core' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'xstore-core' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'xstore-core' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'xstore-core' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'xstore-core' ); ?></button>
				</div>
			</li>
		</script>
		<?php
