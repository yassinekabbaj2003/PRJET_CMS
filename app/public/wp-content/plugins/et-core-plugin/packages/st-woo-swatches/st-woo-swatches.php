...
class St_Woo_Swatches_Base {
	 ...
	 */
	public function get_tax_attribute( $taxonomy, $return_as_array = false ) {
		global $wpdb;
		$origin_attr = substr( $taxonomy, 3 );
        $attribute_type = get_query_var('et_swatch_tax-'.$origin_attr, false);
        $attribute_label = get_query_var('et_swatch_tax-' . $origin_attr.'-label', false);
        if ( !$attribute_label ) { // check only on $attribute_label because $attribute_type value is set in other files
            $attr = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "woocommerce_attribute_taxonomies WHERE attribute_name = %s", $origin_attr ) );
            if ($attr && $attr->attribute_type) {
                $attribute_type = $attr->attribute_type;
                $attribute_label = $attr->attribute_label;
                set_query_var('et_swatch_tax-' . $origin_attr, $attribute_type);
                set_query_var('et_swatch_tax-' . $origin_attr.'-label', $attribute_label);
            }
        }
			...
			foreach ( $cart_item['variation'] as $name => $value ) {
				...
				if( array_key_exists( $attribute_type,  $this->attribute_types ) ) {
					...
					if ( taxonomy_exists( $taxonomy ) ) {
						...
						$data = apply_filters( 'sten_wc_cart_attribute_html', $data, $attribute_type, $term_id, $term_name );
					}
				}
		...
		switch ( $attribute_type ) {
			...
			case 'st-color-swatch':
				...
				$swatch = sprintf('<span class="st-swatch-preview st-color-swatch %1$s">
						<span class="st-custom-attribute" style="%2$s"></span>
					</span>',
