...
class FacebookStrategy extends OpauthStrategy{
	...
	public function int_callback(){
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
			...
			// Use wp function if no response
			if (!$response || apply_filters( 'etheme_force_default_request', false )){
			...
			if (!empty($results) && !empty($results['access_token'])){
				...
				// Use wp function if no response
				if ( apply_filters( 'etheme_force_default_request', false )){
					$me = wp_remote_get( 'https://graph.facebook.com/me', array(
						'timeout'     => 5,
						'redirection' => 5,
						'httpversion' => '1.0',
						'blocking'    => true,
						'headers'     => array(),
						'body'        => array('access_token' => $results->access_token, 'fields' => 'id,name,email,first_name,last_name'),
						'cookies'     => array()
	...
	private function me($access_token){
		$me = $this->serverGet('https://graph.facebook.com/me', array('access_token' => $access_token, 'fields' => 'id,name,email,first_name,last_name'), null, $headers);

		if (empty($me)) {
			$me = etheme_curl_request('https://graph.facebook.com/me', array('access_token' => $access_token, 'fields' => 'id,name,email,first_name,last_name'));
		}
