...
if ( ! current_user_can( 'edit_posts' ) ) {
    wp_die(
        ...
        '<p>' . __( 'Sorry, you are not allowed to see waitlists.' ) . '</p>',
        403
    );
}

$wp_list_table = new XStore_Waitlist_WP_List_Table();
...
<div class="wrap">
    <h1 class="wp-heading-inline">
        <?php
        echo esc_html__( 'Waitlists', 'xstore-core');
        ?>
    </h1>
    ...
    if ( isset( $_REQUEST['error'] ) ) {
        ...
        switch ( $error ) {
            case 1:
                ...
                break;
            case 2:
                $error_msg = __( 'Sorry, you are not allowed to edit on this waitlist.' );
                break;
        }
        if ( $error_msg ) {
    ...
    <form id="waitlist-filter" method="get">

        <input type="hidden" name="page" value="et-waitlists">
