...
class Woothemes_Widget_Testimonials extends WP_Widget {
	...
	public function __construct() {
		...
		$this->woothemes_widget_description = esc_html__( 'Recent testimonials on your site.', 'xstore-core' );
		$this->woothemes_widget_idbase = 'woothemes_testimonials';
		$this->woothemes_widget_title = esc_html__( 'Testimonials', 'xstore-core' );
	...
	public function widget( $args, $instance ) {
		...
		// ! 8theme fix for php v7.2.0+
		if ( ! is_array( $instance['limit'] ) ) $instance = array();
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title (optional):', 'xstore-core' ); ?></label>
			<input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>"  value="<?php echo $instance['title']; ?>" class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" />
		</p>
		<!-- Widget Limit: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php esc_html_e( 'Limit:', 'xstore-core' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php esc_html_e( 'Order By:', 'xstore-core' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php esc_html_e( 'Order Direction:', 'xstore-core' ); ?></label>
		...
		<!-- Widget Display Navigation: Checkbox Input -->
       	<p>
        	...
        	<label for="<?php echo $this->get_field_id( 'navigation' ); ?>"><?php esc_html_e( 'Display Control Navigation', 'xstore-core' ); ?></label>
	   	</p>
		<!-- Widget Display Author: Checkbox Input -->
       	<p>
        	...
        	<label for="<?php echo $this->get_field_id( 'display_author' ); ?>"><?php esc_html_e( 'Display Author', 'xstore-core' ); ?></label>
	   	</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'display_avatar' ); ?>"><?php esc_html_e( 'Display Avatar', 'xstore-core' ); ?></label>
		</p>
		<!-- Slider auto slide -->
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'auto_slide' ); ?>"><?php esc_html_e( 'Auto slide', 'xstore-core' ); ?></label>
		</p>
	   	<!-- Widget Category: Select Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e( 'Category:', 'xstore-core' ); ?></label>
			<?php
				$dropdown_args = array( 'taxonomy' => 'testimonial-category', 'class' => 'widefat', 'show_option_all' => esc_html__( 'All', 'xstore-core' ), 'id' => $this->get_field_id( 'category' ), 'name' => $this->get_field_name( 'category' ), 'selected' => $instance['category'] );
				wp_dropdown_categories( $dropdown_args );
			?>
		</p>
		<!-- Widget ID: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'specific_id' ); ?>"><?php esc_html_e( 'Specific ID (optional):', 'xstore-core' ); ?></label>
		...
		<p><small><?php esc_html_e( 'Display a specific testimonial, rather than a list.', 'xstore-core' ); ?></small></p>
<?php
	} // End form()
	...
	protected function get_orderby_options () {
		return array(
					'none' => esc_html__( 'No Order', 'xstore-core' ),
					'ID' => esc_html__( 'Entry ID', 'xstore-core' ),
					'title' => esc_html__( 'Title', 'xstore-core' ),
					'date' => esc_html__( 'Date Added', 'xstore-core' ),
					'menu_order' => esc_html__( 'Specified Order Setting', 'xstore-core' ),
...
// ! 8theme fix for php v7.2.0+
add_action( 'widgets_init', 'et_register_testimonials_widget' , 1 );

function et_register_testimonials_widget(){
