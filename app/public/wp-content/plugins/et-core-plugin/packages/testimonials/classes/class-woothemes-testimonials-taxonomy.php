...
class Woothemes_Testimonials_Taxonomy {
	...
	public function __construct ( $token = 'testimonial-category', $singular = '', $plural = '', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = esc_html__( 'Category', 'xstore-core' );
		if ( '' == $this->plural ) $this->plural = esc_html__( 'Categories', 'xstore-core' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'xstore-core' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'xstore-core' ), $this->singular ),
			    'search_items'        => sprintf( esc_html__( 'Search %s', 'xstore-core' ), $this->plural ),
			    'all_items'           => sprintf( esc_html__( 'All %s', 'xstore-core' ), $this->plural ),
			    'parent_item'         => sprintf( esc_html__( 'Parent %s', 'xstore-core' ), $this->singular ),
			    'parent_item_colon'   => sprintf( esc_html__( 'Parent %s:', 'xstore-core' ), $this->singular ),
			    'edit_item'           => sprintf( esc_html__( 'Edit %s', 'xstore-core' ), $this->singular ),
			    'update_item'         => sprintf( esc_html__( 'Update %s', 'xstore-core' ), $this->singular ),
			    'add_new_item'        => sprintf( esc_html__( 'Add New %s', 'xstore-core' ), $this->singular ),
			    'new_item_name'       => sprintf( esc_html__( 'New %s Name', 'xstore-core' ), $this->singular ),
			    'menu_name'           => sprintf( esc_html__( '%s', 'xstore-core' ), $this->plural )
			  );
